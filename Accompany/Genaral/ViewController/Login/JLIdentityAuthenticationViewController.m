//
//  JLIdentityAuthenticationViewController.m
//  Accompany
//
...
@implementation JLIdentityAuthenticationViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    [self setNavTitle:@"身份认证"];
...
-(void)saveAction
{
    ...
    [request RequestDataWithUrl:URL_uploadIdentityCardImg pragma:postDict ImageDatas:self.imgData imageName:@"identityCardImg"];
    ...
    request.successBlock = ^(id obj){
        ...
        NSString * identityCardUrl = [obj[0] valueForKey:@"identityCardUrl"];
        
        [UserDefaultsUtils saveValue:identityCardUrl forKey:@"identityCardUrl"];
...
-(UIView*)BgView
{
    if (!_BgView) {
        ...
        _BgView.frame = CGRectMake(0, 64, kMainBoundsWidth, 250*Proportion);
...
-(UIImageView*)IDImageView
{
    if (!_IDImageView) {
        ...
        NSString * identityCardUrl = [UserDefaultsUtils valueWithKey:@"identityCardUrl"];
        
        if (identityCardUrl) {
            [_IDImageView sd_setImageWithURL:[NSURL URLWithString:identityCardUrl]];
        }
...
-(UILabel *)contentLabel
{
    if (!_contentLabel) {
        ...
        _contentLabel.frame = CGRectMake(10, 205*Proportion, kMainBoundsWidth-20, 40);
        _contentLabel.text = @"不要遮挡身份证上面任何信息\n身份证只用于验证身份的真实性,不用于其他用途，其他人不可见";
