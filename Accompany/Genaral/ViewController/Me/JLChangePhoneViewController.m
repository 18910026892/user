...
@implementation JLChangePhoneViewController
...
-(void)setupDatas
{
    _cellTitleArray = @[@"旧手机号",@"原始密码",@"新手机号",@"验证码"];
}
-(void)setupViews
{
...
-(void)SubmitButtonClick:(UIButton*)sender
{
    ...
    CustomTextField * ctf3 = (CustomTextField*)[self.view viewWithTag:1002];
     CustomTextField * ctf4 = (CustomTextField*)[self.view viewWithTag:1003];
    
    [ctf1 resignFirstResponder];
    [ctf2 resignFirstResponder];
    [ctf3 resignFirstResponder];
    [ctf4 resignFirstResponder];
    
    _OldPhoneNumber = [NSString stringWithFormat:@"%@",ctf1.text];
    _OldPassWord = [NSString stringWithFormat:@"%@",ctf2.text];
    _NewPhoneNumber = [NSString stringWithFormat:@"%@",ctf3.text];
    _CodeNumber = [NSString stringWithFormat:@"%@",ctf4.text];
    
    
    NSLog(@"%@___%@___%@___%@",_OldPhoneNumber,_OldPassWord,_NewPhoneNumber,_CodeNumber);
    ...
    }else
    {
        ...
        NSDictionary * postDict = [NSDictionary dictionaryWithObjectsAndKeys:token,@"token",_OldPhoneNumber,@"oldPhone",_OldPassWord,@"password",_NewPhoneNumber,@"newPhone", _CodeNumber,@"code", nil];
        
        HttpRequest * request = [[HttpRequest alloc]init];
        
        [request RequestDataWithUrl:URL_changePhone pragma:postDict];
        
        NSLog(@"%@____%@",postDict,URL_changePhone);
        ...
        request.successBlock = ^(id obj){
            ...
            [HDHud showMessageInView:self.view title:@"更换成功"];
            _UserDict = obj[0];
...
-(void)sendBtnClick:(UIButton*)sender
{
    ...
    CustomTextField * phoneTF  = (CustomTextField*)[self.view viewWithTag:1002];
    _NewPhoneNumber = [NSString stringWithFormat:@"%@",phoneTF.text];
    

    
    if(![_NewPhoneNumber isValidPhone])
    {
    ...
    }else
    {
        ...
        NSDictionary * postDict = [NSDictionary dictionaryWithObjectsAndKeys:@"cphone",@"type",_NewPhoneNumber,@"phone", nil];
        HttpRequest * request = [[HttpRequest alloc]init];
